...
class BottleneckAdapterLayer(nn.Module):
    def __init__(self, config):
        ...
        self.adapter_input_size = config.d_model
...
class MBartEncoderLayer(nn.Module):
    def __init__(self, config: MBartConfig):
        ...
        if config.adapter_latent_size is not None:
            self.adapter_layer = BottleneckAdapterLayer(config)
        else:
    ...
    def forward(
        ...
        if self.adapter_layer is not None:
            hidden_states = self.adapter_layer(hidden_states)
...
class MBartDecoderLayer(nn.Module):
    def __init__(self, config: MBartConfig):
        ...
        if config.adapter_latent_size is not None:
            self.adapter_layer = BottleneckAdapterLayer(config)
            change_requires_grad(self.adapter_layer, True)
        else:
            self.adapter_layer = None
